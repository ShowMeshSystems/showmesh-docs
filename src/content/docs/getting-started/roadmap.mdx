...
<CardGrid>
  ...
  <Card title="Audio node" icon="warning">
    <StatusBadge status="planned" label="Unmerged preview — do not deploy" />

    PR #21 describes the intended audio authority but currently has a broken command path, incomplete LTC work, no output backend, and no hardware evidence. [Read the audio preview →](/guides/set-up-an-audio-node/)
  </Card>
  <Card title="Night-session operation" icon="open-book">
    The operating-day controller is specified and has unmerged implementation work. It is not part of current `main`; its public operator guide remains a day-0 documentation item after a real-installation observation.
  </Card>
</CardGrid>
...
<StatusBadge status="planned" /> means the behavior is design intent or an unmerged preview, not an available operating capability.
