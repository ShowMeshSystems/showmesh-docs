...
- The public API is a contract, not a UI convenience.
...
Repository documents can contradict one another during active development. Resolve the claim against the implementation, tests that would fail if the behavior vanished, the OpenAPI contract for public HTTP behavior, and captured running-system evidence. If uncertainty remains, label it rather than choosing the most polished prose.
