...
<CardGrid>
  ...
  <Card title="Multi-node audio" icon="warning">
    ...
    An installation can declare more than one audio node with a role: program, program-and-LTC, or an independent zone. Cross-node sample sync is not part of this work. [Read about audio nodes →](/using-showmesh/node-types/audio-nodes/)
  </Card>
  <Card title="Signed FPP fallback program" icon="puzzle">
    <StatusBadge status="experimental-active" />

    The coordinator can build and sign a per-FPP fallback program so a previously authorized Cue path can survive a coordinator outage during a scheduled show. This exists on the coordinator today; the FPP-side executor that would act on a delivered program is not established as verified. [Read about FPP →](/integrations/fpp/)
  </Card>
</CardGrid>
...
- published coordinator images and agent packages instead of source builds;
...
- audio hardware commissioning guidance (routing, LTC alignment, receiver lock, failure recovery) and a two-node audio acceptance;
- a packaged FPP plugin installation verified on a real FPP host, including fallback-program execution;
...
- Operator UI documentation once that work settles.
